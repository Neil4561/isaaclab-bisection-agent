...
1. Identify the commit or good/bad range, task, backend, workload size, metric,
   and expected GPU.
2. Run preflight and inspect `preflight.json` plus `hardware_context.json`.
   Hardware mismatch is advisory; preserve it in the report.
3. For one commit, route to `isaaclab-perf-benchmark-commit`. Resolve one
...
4. If benchmark artifacts already exist, route deterministic classification to
   `isaaclab-perf-threshold-check`.
5. For a range, route to `isaaclab-perf-bisect-range`. Use the same pinned
   tooling, task, metric, and warmup contract for every endpoint and candidate.
6. Inspect `measurement_summary.json` for a single commit or `report.md` and
   `summary.json` for a range.
7. Treat categorized skips as evidence. Fix host/environment blockers, but do
   not fall back to benchmark code from an incompatible historical commit.
8. If a serious regression does not reproduce locally, copy
...
6. For a regression fix, verify its regression test fails without the fix.
